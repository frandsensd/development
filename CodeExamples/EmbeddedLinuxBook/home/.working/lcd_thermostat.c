...
unsigned int setpoint, limit, deadband, curtemp;
...
{
	LCD_init();
	LCD_write_string (4, 5, "Temp");
	LCD_write_string (4, 20, "Setpoint");
	LCD_write_string (8, 5, "Limit");
	LCD_write_string (8, 20, "Deadband");
}

void write_screen (void)
{
	LCD_write_number (5, 5, curtemp, 5);
	LCD_write_number (5, 22, setpoint, 5);
	LCD_write_number (9, 5, limit, 5);
	LCD_write_number (9, 22, deadband, 5);
}
